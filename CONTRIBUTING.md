...
7. Follow the [code-signing policy](docs/CODE_SIGNING.md). App Store archives use Xcode Cloud automatic signing only; local development and GitHub CI must not import or select a distribution identity.
8. Do not commit or attach to repository collaboration channels any secret, private key, certificate/private-key bundle, signing certificate, provisioning profile, App Store Connect API key, Keychain, signed release archive, captured camera/audio data, or personal information. Do not send signing material through an issue, pull request, email, or alternate transfer channel; App Store candidates are produced and uploaded only by Xcode Cloud.
