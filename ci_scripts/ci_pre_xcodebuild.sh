...
if [ "$CI_PRODUCT_PLATFORM" != "iOS" ] || \
   [ "$CI_XCODE_SCHEME" != "Bameyasu" ] || \
   [ "$CI_BUNDLE_ID" != "com.hinoshiba.bameyasu" ] || \
...
if [ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' "$repo/Bameyasu/Resources/Info.plist")" != '$(MARKETING_VERSION)' ] || \
   [ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$repo/Bameyasu/Resources/Info.plist")" != '$(CURRENT_PROJECT_VERSION)' ]; then
  echo "error: Bameyasu Info.plist must inherit its version and build from Xcode settings" >&2
  exit 1
fi
...
project_version=$(sed -n 's/^[[:space:]]*MARKETING_VERSION = \([^;]*\);/\1/p' \
  Bameyasu.xcodeproj/project.pbxproj | sort -u)
...
applied_build_number=$(sed -n 's/^[[:space:]]*CURRENT_PROJECT_VERSION = \([^;]*\);/\1/p' \
  Bameyasu.xcodeproj/project.pbxproj | sort -u)
...
echo "Prepared Bameyasu $release_version ($CI_BUILD_NUMBER) for Xcode Cloud archive"
